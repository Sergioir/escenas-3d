 ...
 * POST  { from, to, tipo, contenido, [nombre], [privado] }
 * (tipo 'posicion': respaldo de posición/orientación cuando el P2P de mundo-multi.js no conecta --
 *  contenido es un JSON {x,y,z,yaw} en texto, mismo límite MAX_TXT que un mensaje de chat normal)
 * (tipo 'archivo': adjunto binario -- contenido es un JSON {nombre,mime,datos} en texto, con
 *  "datos" en base64 (data URL); límite MAX_BIN, igual que 'imagen'/'audio')
 * ('privado', booleano opcional: 1 si el mensaje iba dirigido solo a "to" y no a toda la sala --
 *  el propio backend no distingue nada por esto, es solo información para que el cliente que lo
 *  recibe sepa si mostrarlo como privado o como público; ver mundo-multi.js)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $tipo = in_array($data['tipo'] ?? '', ['texto','imagen','audio','sistema','posicion','archivo']) ? $data['tipo'] : null;
    $cont = $data['contenido'] ?? '';
    ...
    $ok = file_put_contents($f, json_encode([
        'from'=>$from,'to'=>$to,'tipo'=>$tipo,'contenido'=>$cont,
        'nombre'=>mb_substr($data['nombre']??'',0,128),
        'privado'=>!empty($data['privado']),
        'ts'=>time(),
    ]), LOCK_EX);
