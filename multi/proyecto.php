...
function fusionarProyectos($base, $local, $servidor) {
    ...
    // "assets": diccionario de contenidos pesados compartidos (imágenes/vídeo/modelos en base64,
    // deduplicados -- ver dedupeAssetInto/resolveProjectAssetRefs en index.html). A diferencia de
    // "objects" (que se fusiona objeto a objeto), aquí basta con UNIR ambos diccionarios: cada entrada
    // va por su id de contenido, así que no hay conflicto real posible entre dos lados -- pero SÍ hace
    // falta la unión (no solo quedarse con el de $local), porque un objeto que sobrevive la fusión de
    // arriba viniendo del lado del SERVIDOR (una alta o edición ajena que este guardado no conocía)
    // puede apuntar a un id de "assets" que solo existe en $servidor, no en $local -- si aquí solo se
    // copiase $local['assets'], esa imagen/modelo quedaría "roto" (referencia sin contenido) tras la
    // fusión.
    $assetsServidor = is_array($servidor['assets'] ?? null) ? $servidor['assets'] : [];
    $assetsLocal = is_array($local['assets'] ?? null) ? $local['assets'] : [];
    if ($assetsServidor || $assetsLocal) {
        $resultado['assets'] = array_merge($assetsServidor, $assetsLocal);
    }
