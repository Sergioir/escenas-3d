 ...
 *
 * POST { sala, proyecto, pin }  -> guarda/actualiza el proyecto de esa sala
 ...
 * GET  ?sala=XXX&comprobarPin=1 -> { ok:true, protegida:true|false }, sin descargar el proyecto
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    if (isset($_GET['comprobarPin'])) {
        echo json_encode(['ok' => true, 'protegida' => file_exists(ficheroPin($sala))]);
        exit;
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $ok = file_put_contents(ficheroProyecto($sala), $proyectoJson, LOCK_EX);
