...
const MundoMulti = (() => {
  ...
  const CHAT_POLL_MS          = 2500;   // solo para el respaldo vía servidor
  const POS_SEND_MS           = 100;    // ~10 Hz — frecuencia de envío de posición
  ...
  let iniciado = false;
  let presenciaTimer = null, signalTimer = null, chatPollTimer = null;
  ...
  function notificarNumPeers() {
    ...
    let n = 0;
    peers.forEach((p) => { if (p.pc && p.pc.connectionState === 'connected') n++; });
    opts.onPeers(n);
  }
  ...
  return { init, entrar, salir, enviarPosicion, enviarChat, actualizarAvatares, numConectados, miSid, estaActivo };
